...
Explanation: "a", "b", "c" appear in order, so the order of "a", "b",
...
def sort_order(order: str, s: str) -> str:
...
def test():
    assert sort_order("cba", "abcd") == "cbad"
    assert sort_order("bcafg", "abcd") == "bcad"
